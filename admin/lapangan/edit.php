...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nama && $jenis && $harga_per_jam > 0) {
        $stmt = $pdo->prepare("UPDATE lapangan SET nama = ?, jenis = ?, harga_per_jam = ?, deskripsi = ?, status = ? WHERE id = ?");
        if ($stmt->execute([$nama, $jenis, $harga_per_jam, $deskripsi, $status, $id])) {
            header("Location: index.php");
            exit;
        } else {
            $error = 'Gagal memperbarui data lapangan.';
        }
    } else {
        $error = 'Harap isi seluruh kolom dengan benar!';
    }
}
?>
<!DOCTYPE html>
...
<body class="bg-slate-950 font-sans min-h-screen flex">
    ...
    <main class="flex-1 p-8 space-y-6 overflow-y-auto max-w-3xl">
        <div class="bg-slate-900 p-6 rounded-2xl border border-slate-700 shadow-sm">
            <h1 class="text-2xl font-black text-white">Edit Data Lapangan #<?= $id ?></h1>
            <p class="text-xs text-slate-400 mt-1">Ubah nama, harga, atau status lapangan.</p>
        </div>
        ...
        <form method="POST" class="bg-slate-900 p-6 rounded-2xl border border-slate-700 shadow-sm space-y-4">
            <div>
                ...
                <input type="text" name="nama" required value="<?= htmlspecialchars($lapangan['nama']) ?>" class="w-full px-4 py-2.5 bg-slate-800 border border-slate-700 rounded-xl font-semibold text-slate-100 outline-none focus:ring-2 focus:ring-green-600">
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="number" name="harga_per_jam" required value="<?= htmlspecialchars($lapangan['harga_per_jam']) ?>" class="w-full px-4 py-2.5 bg-slate-800 border border-slate-700 rounded-xl font-semibold text-slate-100 outline-none focus:ring-2 focus:ring-green-600">
                </div>
            </div>

            <div>
                <label class="block text-xs font-bold text-slate-300 uppercase mb-1">Deskripsi & Fasilitas</label>
                <textarea name="deskripsi" rows="3" class="w-full px-4 py-2.5 bg-slate-800 border border-slate-700 rounded-xl text-xs font-medium text-slate-100 outline-none focus:ring-2 focus:ring-green-600"><?= htmlspecialchars($lapangan['deskripsi']) ?></textarea>
            </div>
            ...
            <div>
                ...
                <select name="status" class="w-full px-4 py-2.5 bg-slate-800 border border-slate-700 rounded-xl font-semibold text-slate-100 outline-none focus:ring-2 focus:ring-green-600">
                    <option value="aktif" <?= $lapangan['status'] === 'aktif' ? 'selected' : '' ?>>Aktif (Tersedia)</option>
                    <option value="nonaktif" <?= $lapangan['status'] === 'nonaktif' ? 'selected' : '' ?>>Nonaktif (Maintenance)</option>
                </select>
            </div>
